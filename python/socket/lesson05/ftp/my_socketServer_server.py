...
#        self.request.send('\033[031;1minvalid_instruction\033[0m')
	...
	while True:
		...
		if re_msg.split()[0] == 'send':
			...
			msg = '\033[031;1mNo file %s found on FTP server \033[0m' % filename
...
P server\nsend filename\tsend file to FTP server\nls\t\tshow file list on FTP server\033[0m'''
		...
		print '%s client %s logout !' %(ctime(),self.client)
		break
try:
    tcpSery = SocketServer.ThreadingTCPServer(ADDR,MyRequestHandler)
    print 'waiting for connecting...'
    tcpSery.serve_forever()
