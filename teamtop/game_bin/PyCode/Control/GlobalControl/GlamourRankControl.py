#!/usr/bin/env python
# -*- coding:UTF-8 -*-
# XRLAM("Control.GlobalControl.GlamourRankControl")
#===============================================================================
# 魅力排行 Control
#===============================================================================
import time
...
if "_HasLoad" not in dir():
	...
	#魅力排行 排行版奖励字典  {(index,serverType):GlamourKuaFuRankConfig,}
	GlamourKuaFuRank_Dict = {}
	#区间对应的排行榜奖励索引 {rankInterval:index,} 
	GlamourKuaFuRankToIndex_Dict = {}
	
class GlamourRankActiveConfig(TabFile.TabLine):
	FilePath = CFILE_FOLDER_PATH.FilePath("GlamourRankActive.txt")
	def __init__(self):
		self.activeName = str
	...
	def init_active(self):
		...
		if self.startTime <= nowDate < self.endTime:
			#在开始和结束时间戳之间, 激活
			OpenGlamourRank(None, endTime)
			cComplexServer.RegTick(endTime - nowTime + 60, CloseGlamourRank)
		elif nowDate < self.startTime:
			#在开始时间戳之前
			cComplexServer.RegTick(beginTime - nowTime, OpenGlamourRank, endTime)
			cComplexServer.RegTick(endTime - nowTime + 60, CloseGlamourRank)

def LoadGlamourRankActiveConfig():
	for cfg in GlamourRankActiveConfig.ToClassType(False):
		if cfg.startTime > cfg.endTime:
			print "GE_EXC, startTime > endTime in GlamourRankActive"
			return
		cfg.init_active()
		
class GlamourKuaFuRankConfig(TabFile.TabLine):
	FilePath = FILE_FOLDER_PATH.FilePath("GlamourKuaFuRank.txt")
	def __init__(self):
		self.index = int					#排名奖励索引
		self.rankInterval = eval			#排名区间
		self.serverType = int				#服务器类型
		self.needGlamour = int				#占位最低魅力值
		self.reward = eval					#奖励
	
def LoadGlamourKuaFuRankConfig():
	global GlamourKuaFuRank_Dict
	global GlamourKuaFuRankToIndex_Dict
	
	for cfg in GlamourKuaFuRankConfig.ToClassType(False):
		if (cfg.index, cfg.serverType) in GlamourKuaFuRank_Dict:
			print "GE_EXC, repeat (index : %s, serverType : %s) in GlamourKuaFuRank_Dict" % (cfg.index, cfg.serverType)
			continue
		#排名区间 -- > 排名索引
		if cfg.rankInterval not in GlamourKuaFuRankToIndex_Dict:
			GlamourKuaFuRankToIndex_Dict[cfg.rankInterval] = cfg.index
		GlamourKuaFuRank_Dict[(cfg.index, cfg.serverType)] = cfg
...
def GetRankCfgByRankAndType(rank, serverType):
	...
	'''
	retIndex = 0
	for rankInterval, index in GlamourKuaFuRankToIndex_Dict.iteritems():
		rankDown , rankUp = rankInterval
		if rankDown <= rank and rank <= rankUp:
			retIndex = index
	
	rankCfg = GlamourKuaFuRank_Dict.get((retIndex,serverType))
	return rankCfg
	
def OpenGlamourRank(callArgv, regparam):
	#开启魅力排行
	global IS_START
	if IS_START:
		print "GE_EXC, GlamourRank is already open"
		return
	IS_START = True

def CloseGlamourRank(callArgv, regparam):
	#结束魅力排行
	global IS_START
	if not IS_START:
		print "GE_EXC, GlamourRank is already close"
		return
...
def AfterNewMinute():
	...
	if ReturnDB is False:
		print "GE_EXC, GlamourRankControl AfterNewHour error not ReturnDB"
		#数据没有载回, 尝试再次载入
		InitGetRankEx()
		return
...
def RequestLogicRank():
	...
	for sessionid, cp in ProcessMgr.ControlProcesssSessions.iteritems():
		if cp.processid >= 30000:
			...
			TotalLogicCnt -= 1
			continue
		ControlProxy.SendLogicMsgAndBack(sessionid, PyMessage.Control_RequestLogicGlamourRank, None, LogicBackRank, sessionid)
...
def LogicBackRank(callargv, regparam):
	...
	if TotalLogicCnt == 0:
		...
		print "GE_EXC, error in GlamourRankControl RequestBack (%s)" % TotalLogicCnt
...
def SortAndUpdataRank():
	...
	if RankCache_1:
		...
		GlobalHttp.SetGlobalData(GlobalDataDefine.GlamourRankDataKey_1, (days, RankCache_1))
	...
	if RankCache_2:
		...
		GlobalHttp.SetGlobalData(GlobalDataDefine.GlamourRankDataKey_2, (days, RankCache_2))
	...
	if RankCache_3:
		...
		GlobalHttp.SetGlobalData(GlobalDataDefine.GlamourRankDataKey_3, (days, RankCache_3))
	...
	if RankCache_4:
		...
		GlobalHttp.SetGlobalData(GlobalDataDefine.GlamourRankDataKey_4, (days, RankCache_4))
	...
	if RankCache_5:
		...
		GlobalHttp.SetGlobalData(GlobalDataDefine.GlamourRankDataKey_5, (days, RankCache_5))
...
def NewDayRewardRoles():
	...
	#更新跨服数据到数据库中
	GlobalHttp.SetGlobalData(GlobalDataDefine.GlamourRankDataKey_old_1, (days - 1, RankCache_old_1))
	GlobalHttp.SetGlobalData(GlobalDataDefine.GlamourRankDataKey_old_2, (days - 1, RankCache_old_2))
	GlobalHttp.SetGlobalData(GlobalDataDefine.GlamourRankDataKey_old_3, (days - 1, RankCache_old_3))
	GlobalHttp.SetGlobalData(GlobalDataDefine.GlamourRankDataKey_old_4, (days - 1, RankCache_old_4))
	GlobalHttp.SetGlobalData(GlobalDataDefine.GlamourRankDataKey_old_5, (days - 1, RankCache_old_5))
...
def RewardRolesByServerType(rankData, serverType):
	...
	maildatas = []	
	transaction = AutoLog.traGlamourRankMailReward
	for roleid, rank in tmpRankDict.iteritems():
		rolemaildata = (roleid, GlobalPrompt.GlamourRank_Title, GlobalPrompt.GlamourRank_Sender, GlobalPrompt.GlamourRank_Content % rank, transaction, GetRankMailReward(rank, serverType))
...
def GetRankMailReward(rank, serverType):
	...
	global GlamourKuaFuRankToIndex_Dict
	index = 0
	for (begin, end), rewardIndex in GlamourKuaFuRankToIndex_Dict.iteritems():
	...
	#(奖励索引, 服务器类型) --> 奖励
	cfg = GlamourKuaFuRank_Dict.get((index, serverType))
	if not cfg:
...
def InitGetRankEx():
	index = 1
	key = GlobalDataDefine.GlamourRankDataKey_1
	oldkey = GlobalDataDefine.GlamourRankDataKey_old_1
	GlobalHttp.GetGlobalDataByKeys([key, oldkey], OnGetRankBackEx, (index, key, oldkey))
	
	index = 2
	key = GlobalDataDefine.GlamourRankDataKey_2
	oldkey = GlobalDataDefine.GlamourRankDataKey_old_2
	GlobalHttp.GetGlobalDataByKeys([key, oldkey], OnGetRankBackEx, (index, key, oldkey))

	index = 3
	key = GlobalDataDefine.GlamourRankDataKey_3
	oldkey = GlobalDataDefine.GlamourRankDataKey_old_3
	GlobalHttp.GetGlobalDataByKeys([key, oldkey], OnGetRankBackEx, (index, key, oldkey))

	index = 4
	key = GlobalDataDefine.GlamourRankDataKey_4
	oldkey = GlobalDataDefine.GlamourRankDataKey_old_4
	GlobalHttp.GetGlobalDataByKeys([key, oldkey], OnGetRankBackEx, (index, key, oldkey))

	index = 5
	key = GlobalDataDefine.GlamourRankDataKey_5
	oldkey = GlobalDataDefine.GlamourRankDataKey_old_5
	GlobalHttp.GetGlobalDataByKeys([key, oldkey], OnGetRankBackEx, (index, key, oldkey))
...
def SyncAllLogin():
	...
	for processId, pType in LogicType_Dict.iteritems():
		...
		if pType == 1:
			ControlProxy.SendLogicMsg(sessionid, PyMessage.Control_UpdataGlamourRankToLogic, (RankCache_1, RankCache_old_1))
		elif pType == 2:
			ControlProxy.SendLogicMsg(sessionid, PyMessage.Control_UpdataGlamourRankToLogic, (RankCache_2, RankCache_old_2))
		elif pType == 3:
			ControlProxy.SendLogicMsg(sessionid, PyMessage.Control_UpdataGlamourRankToLogic, (RankCache_3, RankCache_old_3))
		elif pType == 4:
			ControlProxy.SendLogicMsg(sessionid, PyMessage.Control_UpdataGlamourRankToLogic, (RankCache_4, RankCache_old_4))
		elif pType == 5:
			ControlProxy.SendLogicMsg(sessionid, PyMessage.Control_UpdataGlamourRankToLogic, (RankCache_5, RankCache_old_5))
		else:
...
def SyncAllLoginToday():
	...
	for processId, pType in LogicType_Dict.iteritems():
		...
		sessionid = cp.session_id
		if pType == 1:
			ControlProxy.SendLogicMsg(sessionid, PyMessage.Control_UpdataGlamourRankToLogic_T, RankCache_1)
		elif pType == 2:
			ControlProxy.SendLogicMsg(sessionid, PyMessage.Control_UpdataGlamourRankToLogic_T, RankCache_2)
		elif pType == 3:
			ControlProxy.SendLogicMsg(sessionid, PyMessage.Control_UpdataGlamourRankToLogic_T, RankCache_3)
		elif pType == 4:
			ControlProxy.SendLogicMsg(sessionid, PyMessage.Control_UpdataGlamourRankToLogic_T, RankCache_4)
		elif pType == 5:
			ControlProxy.SendLogicMsg(sessionid, PyMessage.Control_UpdataGlamourRankToLogic_T, RankCache_5)
		else:
			print "GE_EXC, SyncAllLogin error pType (%s)" % pType
...
if "_HasLoad" not in dir():
	if Environment.HasControl:
		...
		LoadGlamourKuaFuRankConfig()
		InitGetRankEx()
		
		cComplexServer.RegAfterNewMinuteCallFunction(AfterNewMinute)
		cComplexServer.RegDistribute(PyMessage.Control_GetGlamourRank, LoginRequestRank)