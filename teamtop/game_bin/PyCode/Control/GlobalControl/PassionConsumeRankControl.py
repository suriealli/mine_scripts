#!/usr/bin/env python
# -*- coding:UTF-8 -*-
# XRLAM("Control.GlobalControl.PassionConsumeRankControl")
#===============================================================================
# 激情活动 -- 消费排名控制
#===============================================================================
import copy
...
if "_HasLoad" not in dir():
	...
	#积分排行—— 排行版奖励字典  {(index,levelRangeId):PassionConsumeRankConfig,}
	PassionConsumeKuaFuRank_Dict = {}
	#区间对应的排行榜奖励索引 {rankInterval:index,} 
	PassionConsumeKuaFuRankToIndex_Dict = {}
	#等级段关联等级段ID{levelRangeId:levelRange,}
	PassionConsumeKuaFuRankLevelID2Range_Dict = {}
	
class PassionConsumeRankActiveConfig(TabFile.TabLine):
	FilePath = CFILE_FOLDER_PATH.FilePath("PassionConsumeRankActive.txt")
	def __init__(self):
	...
	def init_active(self):
		...
		if beginTime <= nowTime < endTime:
			#在开始和结束时间戳之间, 激活
			OpenPassionConsumeRank(None, endTime)
			cComplexServer.RegTick(endTime - nowTime + 60, ClosePassionConsumeRank)
		elif nowTime < beginTime:
			#在开始时间戳之前
			cComplexServer.RegTick(beginTime - nowTime, OpenPassionConsumeRank, endTime)
			cComplexServer.RegTick(endTime - nowTime + 60, ClosePassionConsumeRank)

def LoadPassionConsumeRankActiveConfig():
	for cfg in PassionConsumeRankActiveConfig.ToClassType(False):
		if cfg.startTime > cfg.endTime:
			print "GE_EXC, startTime > endTime in PassionConsumeRankActive"
			return
		cfg.init_active()
		
class PassionConsumeKuaFuRankConfig(TabFile.TabLine):
	FilePath = FILE_FOLDER_PATH.FilePath("PassionConsumeKuaFuRank.txt")
	def __init__(self):
		self.index = int							#排名奖励索引
		self.rankInterval = eval					#排名区间
		self.needRMB = int							#占位最低消费
		self.levelRangeId = int						#等级段ID
		self.levelRange = self.GetEvalByString		#等级段
		self.rewardItems = eval							#奖励
	
def LoadPassionConsumeKuaFuRankConfig():
	global PassionConsumeKuaFuRank_Dict, PassionConsumeKuaFuRankToIndex_Dict, PassionConsumeKuaFuRankLevelID2Range_Dict
	
	for cfg in PassionConsumeKuaFuRankConfig.ToClassType(False):
		index = cfg.index
		levelRangeId = cfg.levelRangeId
		levelRange = cfg.levelRange
		rankInterval = cfg.rankInterval
		if (index, levelRangeId) in PassionConsumeKuaFuRank_Dict:
			print "GE_EXC, repeat (index : %s, levelRangeId:%s) in PassionConsumeKuaFuRank_Dict" % (index, levelRangeId)
			
		#排名区间 -- > 排名索引
		if rankInterval not in PassionConsumeKuaFuRankToIndex_Dict:
			PassionConsumeKuaFuRankToIndex_Dict[rankInterval] = index
			
		PassionConsumeKuaFuRank_Dict[(index, levelRangeId)] = cfg
		
		PassionConsumeKuaFuRankLevelID2Range_Dict[levelRangeId] = levelRange
...
def GetRankCfgByRankAndType(rank, roleLevel):
	...
	'''
	retIndex = 0
	for rankInterval, index in PassionConsumeKuaFuRankToIndex_Dict.iteritems():
		rankDown , rankUp = rankInterval
		if rankDown <= rank and rank <= rankUp:
			retIndex = index
	
	tmpLevelRangeId = 0
	for levelRangeId, levelRange in PassionConsumeKuaFuRankLevelID2Range_Dict.iteritems():
		tmpLevelRangeId = levelRangeId
		if levelRange[0] <= levelRange[1]:
			break
	
	rankCfg = PassionConsumeKuaFuRank_Dict.get((retIndex, tmpLevelRangeId))
	return rankCfg
	
def OpenPassionConsumeRank(callArgv, regparam):
	#开启积分排行
	global IS_START
	if IS_START:
		print "GE_EXC, PassionConsumeRank is already open"
		return
	IS_START = True

def ClosePassionConsumeRank(callArgv, regparam):
	#结束积分排行
	global IS_START
	if not IS_START:
		print "GE_EXC, PassionConsumeRank is already close"
		return
...
def AfterNewMinute():
	...
	if ReturnDB is False:
		print "GE_EXC, PassionConsumeRankControl AfterNewHour error not ReturnDB"
		#数据没有载回, 尝试再次载入
		InitGetRankEx()
		return
...
def RequestLogicRank():
	...
	for sessionid, cp in ProcessMgr.ControlProcesssSessions.iteritems():
		if cp.processid >= 30000:
			...
			TotalLogicCnt -= 1
			continue
		ControlProxy.SendLogicMsgAndBack(sessionid, PyMessage.Control_RequestLogicPassionConsumeRank, None, LogicBackRank, sessionid)
...
def LogicBackRank(callargv, regparam):
	...
	if TotalLogicCnt == 0:
		...
		print "GE_EXC, error in PassionConsumeRankControl RequestBack (%s)" % TotalLogicCnt
...
def SortAndUpdataRank():
	...
	if RankCache:
		...
		GlobalHttp.SetGlobalData(GlobalDataDefine.PassionConsumeRankDataKey, (days, RankCache))
...
def NewDayRewardRoles():
	...
	#更新跨服数据到数据库中
	GlobalHttp.SetGlobalData(GlobalDataDefine.PassionConsumeRankDataKey_old, (days - 1, RankCache_old))
	GlobalHttp.SetGlobalData(GlobalDataDefine.PassionConsumeRankDataKey, (days, RankCache))
...
def RewardRolesByServerType(rankData):
	...
	maildatas = []	
	transaction = AutoLog.traPassionConsumeRankMailReward
	for roleid, rankInfo in tmpRankDict.iteritems():
		rank, roleLevel = rankInfo
		rolemaildata = (roleid, GlobalPrompt.PassionConsumeRank_KuaFu_Title, GlobalPrompt.PassionConsumeRank_KuaFu_Sender, GlobalPrompt.PassionConsumeRank_KuaFu_Content % (cDateTime.Year(), cDateTime.Month(), cDateTime.Day(), rank), transaction, GetRankMailReward(rank, roleLevel))
		maildatas.append(rolemaildata)
	#发送邮件奖励
	GlobalHttp.SendRoleMail(maildatas)
	
	print 'BLUE, PassionConsumeKuaFuRank %s' % tmpRankDict
...
def GetRankMailReward(rank, roleLevel):
	...
	#排名 --> 排名区间 --> 奖励索引
	global PassionConsumeKuaFuRankToIndex_Dict
	index = 0
	for (begin, end), rewardIndex in PassionConsumeKuaFuRankToIndex_Dict.iteritems():
	...
	for levelRangeId, levelRange in PassionConsumeKuaFuRankLevelID2Range_Dict.iteritems():
		...
		print 'GE_EXC, PassionConsumeKuaFuRank can not find %s level range id' % roleLevel
		return
	
	#(奖励索引, 服务器类型) --> 奖励
	cfg = PassionConsumeKuaFuRank_Dict.get((index, tmpLevelRangeId))
	if not cfg:
...
def InitGetRankEx():
	#数据太大了, 要分开载入
	key = GlobalDataDefine.PassionConsumeRankDataKey
	oldkey = GlobalDataDefine.PassionConsumeRankDataKey_old
...
def OnGetRankBackEx(response, regparam):
	...
	#处理数据
	days = cDateTime.Days()
	Checkdata(LoadDataDict, days, GlobalDataDefine.PassionConsumeRankDataKey, GlobalDataDefine.PassionConsumeRankDataKey_old)
...
def SyncAllLogin():
	...
	for processId in LoginRankAll_Dict.iterkeys():
		...
		ControlProxy.SendLogicMsg(sessionid, PyMessage.Control_UpdataPassionConsumeRankToLogic, (RankCache, RankCache_old))
...
def SyncAllLoginToday():
	...
	for processId in LoginRankAll_Dict.iterkeys():
		...
		if not cp:
			...
			continue
		sessionid = cp.session_id
		ControlProxy.SendLogicMsg(sessionid, PyMessage.Control_UpdataPassionConsumeRankToLogic_T, RankCache)
...
if "_HasLoad" not in dir():
	if Environment.HasControl:
		...
		LoadPassionConsumeKuaFuRankConfig()
		InitGetRankEx()
		
		cComplexServer.RegAfterNewMinuteCallFunction(AfterNewMinute)
		cComplexServer.RegDistribute(PyMessage.Control_GetPassionConsumeRank, LoginRequestRank)
