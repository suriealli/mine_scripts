#!/usr/bin/env python
# -*- coding:UTF-8 -*-
# XRLAM("Game.Activity.HefuBoss.HefuBossTeam")
#===============================================================================
# 合服boss组队相关
#===============================================================================
...
if "_HasLoad" not in dir():
	Sync_HefuBoss_SeverTeams_Data = AutoMessage.AllotMessage("Sync_HefuBoss_SeverTeams_Data", "同步合服boss全服组队信息")
	Show_HefuBoss_Invite_Data = AutoMessage.AllotMessage("Show_HefuBoss_Invite_Data", "通知客户端显示合服boss组队邀请信息")
	Show_HefuBoss_Apply_Leader_Panel = AutoMessage.AllotMessage("Show_HefuBoss_Apply_Leader_Panel", "通知客户端显示合服boss申请队长面板")
...
def CanCreateTeam(role, teamType):
	...
	if not Status.CanInStatus(role, EnumInt1.ST_Team):
		return False
	#组队类型是否合法
	if teamType != EnumTeamType.T_HefuBoss:
		return False
...
def ApplyTeamLeader(role, backFunId):
	...
	if role not in team.members:
		return
	#通知队长
	team.leader.SendObjAndBack(Show_HefuBoss_Apply_Leader_Panel, role.GetRoleName(), 80, OnBackApplyLeader, role.GetRoleID())
...
def TeamInvite(role, desRoleId):
	...
	#被邀请是否满足组队等级条件
	if desRole.GetLevel() < EnumGameConfig.HefuBossNeedLevel:
		return
	...
	#队伍id, 玩家名, 副本ID
	desRole.SendObjAndBack(Show_HefuBoss_Invite_Data, (team.team_id, role.GetRoleName(), role.GetI8(EnumInt8.UnionFBId)), 120, InviteBack, (team.team_id, sceneId, x, y))
...
def ShowTeams(role):
	...
	@param role:
	'''
	teamlist = TeamBase.HEFUBOSS_TEAM_LIST
	sendList = []
	...
	#同步客户端
	role.SendObj(Sync_HefuBoss_SeverTeams_Data, sendList)
...
if "_HasLoad" not in dir():
	if Environment.HasLogic and not Environment.IsCross:
		#消息
		cRoleMgr.RegDistribute(AutoMessage.AllotMessage("HefuBoss_Team_Create", "客户端请求创建合服boss队伍"), RequestCreateTeam)
		cRoleMgr.RegDistribute(AutoMessage.AllotMessage("HefuBoss_Team_Dismiss", "客户端请求解散合服boss队伍"), RequestDismissTeam)
		cRoleMgr.RegDistribute(AutoMessage.AllotMessage("HefuBoss_Team_Join", "客户端请求加入合服boss队伍"), RequestJoinTeam)
		cRoleMgr.RegDistribute(AutoMessage.AllotMessage("HefuBoss_Team_Quit", "客户端请求离开合服boss队伍"), RequestQuitTeam)
		cRoleMgr.RegDistribute(AutoMessage.AllotMessage("HefuBoss_Team_Kick", "客户端请求踢人出合服boss队伍"), RequestKickTeamMember)
		cRoleMgr.RegDistribute(AutoMessage.AllotMessage("HefuBoss_Team_Transfer_Leader", "客户端请求转让合服boss队长"), RequestTransferTeamLeader)
		cRoleMgr.RegDistribute(AutoMessage.AllotMessage("HefuBoss_Team_Apply_Leader", "客户端请求申请合服boss队长"), RequestApplyTeamLeader)
		cRoleMgr.RegDistribute(AutoMessage.AllotMessage("HefuBoss_Team_Change_Pos", "客户端请求请求改变合服boss队伍队员位置"), RequestTeamChangePos)
		cRoleMgr.RegDistribute(AutoMessage.AllotMessage("HefuBoss_Team_Invite", "客户端请求邀请玩家加入合服boss队伍"), RequestTeamInvite)
		cRoleMgr.RegDistribute(AutoMessage.AllotMessage("HefuBoss_Get_Teams", "客户端请求获取合服boss当前队伍信息"), RequestGetTeams)
