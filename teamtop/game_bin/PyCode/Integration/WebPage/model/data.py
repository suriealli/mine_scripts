...
def check(R):
	data={}
	get={}
	get['check']=me.G(R,'check')
	get['server']=me.G(R,'server')
	get['targets']=me.G(R,'column','list')
	get['start_date']=me.G(R,'start_date')
	get['end_date']=me.G(R,'end_date')
	get['nogroup']=me.G(R,'nogroup')
	get['no_row_group']=me.G(R,'no_row_group')
	get['update']=me.G(R,'update')
	get['servers']=me.G(R,'servers','list')
	
	#set default param
	if not get['start_date']:
		get['start_date']=tool.date(time.time()-86400)
	if not get['end_date']:
		get['end_date']=tool.date(time.time()-86400)
	data['servers']=server.getAll()
	data['targets']=targets
	data['functions']=functions
	data['get']=get
	data['got_targets']=json.dumps(get['targets'])
	data['targets_info']=json.dumps(targets)
	
	#no target in,just return
	if len(get['targets'])==0:
		data['tips']="请选择需要数据！"
		return	data
	
	if targets[get['targets'][0]]['table']=='cache' and get['server']=='all':
		get['no_row_group']='true' #cache data need no group
	target_count=len(get['targets'])
	temp=getData(get)
...
def manageAllData(R):
	...
	if get['step']:
		if get['step']=='get':
			log=getAllData(R)
			data['tips']=log
	if 'time_start' not in R.POST:
		get['time_start']='%s 00:00:00'%tool.date(time.time())
	if 'time_end' not in R.POST:
		get['time_end']='%s 23:59:59'%tool.date(time.time())
	data['get']=get
	return data
...
def getAllData(R):
	...
	get['servers']=me.G(R,'servers','list')
	get['time_start']=me.G(R,'time_start')
	get['time_end']=me.G(R,'time_end')
	get['memory']=me.G(R,'memory')
	#auto update last
	get['auto']=me.G(R,'auto','int')
	if get['auto']>0:
		now_time=time.time()
		get['time_start']=tool.date(now_time-get['auto']*60,"%Y-%m-%d %H:%M:%S")
		get['time_end']=tool.date(now_time,"%Y-%m-%d %H:%M:%S")
	#check
	get['check']=me.G(R,'check')
	
	get['models']=me.G(R,'models')
	log.append('$$Need Servers(%s) in (%s - %s)'%(','.join(get['servers']),get['time_start'],get['time_end']))
	#汇总库的存储体
	if not get['memory']:
		get['memory']=me.DBS['total']
	#获取所有model
	models={
		'role_data':{
	...
	for k,v in servers.items():
		if get['servers'] and str(k) not in get['servers']:
			log.append("* %s not in [%s]"%(k,",".join(get['servers'])))
			continue
		log.append('【SERVER:%s】'%k)
		#遍历所有汇总表
		for model_key,model in models.items():
			if get['models'] and model_key not in get['models'].split(','):
				log.append('## SKIP MODEL:%s ##'%model_key)
				continue
			log.append('== NOW MODEL:%s =='%model_key)
			# #get last entity
			# entity_last=me.M("recall",{"memory":get['memory'],"one":True,"model":model_key,"field":model['property'][0],"option":{"order":"%s,desc"%model['property'][0]},"range":"1,1"})
			# #get all new entity
			# entity_last_id=0
			# log.append('#Last Entity:%s'%entity_last_id)
			# if entity_last and entity_last.get(model['property'][0],0)>0:
				# entity_last_id=entity_last[model['property'][0]]
			#sql
			#filter='`%s`>%s'%(model['property'][0],entity_last_id)
			
			#forget entity in time if not insert before check
			filter='%s between "%s" and "%s"'%(model['spliter'],get['time_start'],get['time_end'])
			if not get['check']:
				log.append('-- FORGET OLD DATA --')
				me.M("forget",{"memory":get['memory'],"model":model_key,"filter":"%s and `server`=%s"%(filter,k)})
			
			temp=me.M("recall",{"memory":"server:%s"%k,"model":model_key,"field":'%s'%(','.join(model['property'])),"filter":filter})
			#add entity to total
			count=0
			error=[]
			exist=[]
				...
				try:
					if get['check']:
						...
						for fv in model['ids']:
							filter[fv]=entity[fv]
						filter['server']=k
						if me.M("exist",{"memory":get['memory'],"model":model_key,"filter":filter}):
							exist.append(str(entity[model['ids'][0]]))
							continue
					me.M("add",{"memory":get['memory'],"model":model_key,"data":entity})
				except:
