	...
	for role_id in msg['role_id']:
		...
		try:
			...
			if command != "":
				...
				sql = 'update role_data set di32_3=0 where role_id=%s' % (role_id)
				con = db.link({"db": "role:%s" % role_id})
				with con as cur:
...
def sendCommand(role_id,m):
	from ComplexServer.Plug.DB import DBHelp
	con = db.link({"db":"role:%s" % role_id, "dict":False})
	done = False
	try:
...
def sendMail(msg=None):
	...
	for sv in servers:
		...
		if str(msg['role_id']) != "5201314":
			...
			con = db.link({"dict": False, "db": "server:%s" % sv})
			with con as cur:
				#send to each role
	...
	if str(msg['role_id']) == "5201314":
		#创建目录
		#absolute path
		log_path = os.path.abspath(os.path.dirname(__file__)).replace("%spf" % os.sep,"%spf%slog" % (os.sep, os.sep))
		try:
